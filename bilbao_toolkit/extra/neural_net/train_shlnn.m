function [shlnn,mce,penW,penU,llh] = train_shlnn(X,labels,nhid,rW,rU,prior,maxiters,timeout)
...
%   llh: training class log-liklehoods: llh = shlnn(X)
    ...
    [W,a,U,b] = vec2params(w,params0);
    shlnn = @(X) shlnnBlock(W,a,U,b,X);
    llh = shlnn(X);
    mce = obj(llh);
    penW = L2reg(W,rW);
    penU = L2reg(U,rU);
    ...
    function [y,grad,GN] = shlnn_obj(w)
        ...
        %fwd pass
        [W,a,U,b] = vec2params(w,params0);
        [llh,backX,fwdX] = shlnnBlock(W,a,U,b,X);
